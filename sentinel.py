...
def _compute_fleet_exit(m: dict) -> bool:
    ...
    short_cas = sum(1 for v in last10 if v <= -0.008) >= 6   # sustained negative velocity
    long_squ  = sum(1 for v in last10 if v >=  0.008) >= 6   # sustained positive velocity
    if not (short_cas or long_squ):
        return False
    last2 = recent[-2:]
    return all(abs(v) < 0.005 for v in last2)   # velocity draining to zero
...
def get_metrics() -> dict:
    """Phase A: expose sentinel state for /api/state. Phase B/C: convergence + exit signal."""
    return {
        "regime":                  _regime,
