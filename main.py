...
def _save_state():
    ...
    try:
        ...
        sb.table("bybit_scanner_state").upsert(data).execute()
    except Exception as _e:
...
def _load_state():
    ...
    try:
        ...
        log_rows = sb.table("bybit_trade_log").select("*").order("created_at").limit(1000).execute()
        if log_rows.data:
        ...
        result = sb.table("bybit_scanner_state").select("*").eq("id", 1).execute()
        if not result.data:
...
def _append_trade_log(trade: dict, exit_price: float, reason: str, pnl: float, r: float):
    ...
    if sb is not None:
        try:
            ...
            print(f"[PERSIST] bybit_trade_log insert error: {_e}")
...
async def _open_trade_log_row(trade: dict):
    """Insert an entry-analytics snapshot into bybit_trade_log at trade-open time.

    If any column is missing, run once in Supabase SQL editor:
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS j15m_entry       float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS j1h_entry        float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS stoch_k_entry    float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS stoch_d_entry    float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS rsi_entry        float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS depth_pct_entry  float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS chg24h_entry     float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS session_opened   text;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS mae_r            float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS mfe_r            float;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS score           integer;
      ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS adx1h           float;
    """
    sb = _get_supabase()
    if not sb:
        return
    try:
        ...
        open_iso  = datetime.fromtimestamp(opened_at, tz=timezone.utc).isoformat()
        sb.table("bybit_trade_log").insert({
            "pair":            trade["symbol"],
            "direction":       trade["direction"],
            "tier":            trade.get("tier"),
            "leverage":        trade.get("leverage"),
        ...
        print(f"[TRADE LOG OPEN] {trade['symbol']} {trade['direction']} open-row written to bybit_trade_log")
    except Exception as _e:
        print(f"[TRADE LOG WRITE FAILED] bybit_trade_log open-row: {_e}")
        ...
        try:
            ...
            if _prev_session and _curr_sess != _prev_session:
                ...
                try:
                    ...
                    if _sb_sr:
                        _sb_sr.table("bybit_scanner_state").update(
...
def _do_partial_close_profit_lock(key: str, trade: dict, exit_price: float, close_pct: float, tier: str):
    ...
    print(f"[{tier}] OKX {sym} {direction} partial lock {pct_int}% at {exit_price} "
        ...
        for key, trade in list(app_state.open_trades.items()):
            try:
                ...
                if (not tp1_hit
                    ...
                    print(f"[SKIM] OKX {sym} {direction}"
                ...
                if (not tp1_hit
                        ...
                        and _pl_cur_r >= 0.02):
                    print(f"[LOCK] OKX {sym} {direction}"
                          f" peak_r={_pl_peak_r:.3f}R cur_r={_pl_cur_r:.3f}R"
                          f" giveback={_pl_giveback:.0%} cpnl={_cpnl:.2f}")
                    ...
                    if _bte_fired:
                        ...
                        if _session_sl_counts[_skey] >= 2 and _skey not in _session_halted:
                            ...
                            try:
                                ...
                                if _sb_dh:
                                    _sb_dh.table("bybit_scanner_state").update(
...
async def lifespan(app: FastAPI):
    ...
    await _resolve_bot_identity("BYBIT")
    _sentinel_mod.init("BYBIT", _get_supabase)
    print("[SENTINEL] Phase 0 watchdog initialized -- observe-only")
    print("[SCHEMA] bybit_trade_log analytics columns Ã¢ÂÂ run once in Supabase SQL editor if any are missing:")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS j15m_entry       float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS j1h_entry        float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS stoch_k_entry    float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS stoch_d_entry    float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS rsi_entry        float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS depth_pct_entry  float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS chg24h_entry     float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS session_opened   text;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS mae_r            float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS mfe_r            float;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS score           integer;")
    print("  ALTER TABLE bybit_trade_log ADD COLUMN IF NOT EXISTS adx1h           float;")
...
async def get_state():
    ...
    try:
        ...
        _fh_r  = _fh_sb.table("bybit_scanner_state").select("fleet_halt").eq("id", 1).execute() if _fh_sb else None
    ...
    if from_ts is not None and to_ts is not None:
        ...
        if sb is not None:
            try:
                ...
                sb.table("bybit_trade_log").delete() \
